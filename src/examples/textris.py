...
for p in PIECES:  # loop through each piece
    for r in range(len(PIECES[p])):  # loop through each rotation of the piece
        ...
        for x in range(
            TEMPLATEWIDTH
        ):  # loop through each column of the rotation of the piece
            column = []
            assert (
                len(PIECES[p][r]) == TEMPLATEWIDTH
            ), "Malformed shape given for piece %s, rotation %s" % (p, r)
            for y in range(
                TEMPLATEHEIGHT
            ):  # loop through each character in the column of the rotation of the piece
                assert (
                    len(PIECES[p][r][y]) == TEMPLATEHEIGHT
                ), "Malformed shape given for piece %s, rotation %s" % (p, r)
                if PIECES[p][r][y][x] == ".":
                    column.append(BLANK)
                else:
...
def hasHitBottom(board, piece):
    ...
    for x in range(TEMPLATEWIDTH):
        for y in range(TEMPLATEHEIGHT):
            if (
                PIECES[piece["shape"]][piece["rotation"]][x][y] == BLANK
                or y + piece["y"] + 1 < 0
            ):
...
def isValidPosition(board, piece, adjX=0, adjY=0):
    ...
    for x in range(TEMPLATEWIDTH):
        for y in range(TEMPLATEHEIGHT):
            if (
                y + piece["y"] + adjY < 0
                or PIECES[piece["shape"]][piece["rotation"]][x][y] == BLANK
            ):
                continue
...
def drawStatus(score, level):
    # draw the score text
    WINDOWSURF.putchars(
        "Score:", x=WINDOWWIDTH - 8, y=2, fgcolor="gray", bgcolor="black"
    )
    WINDOWSURF.putchars(
        str(score), x=WINDOWWIDTH - 7, y=3, fgcolor="white", bgcolor="black"
    )
    # draw the level text
    WINDOWSURF.putchars(
        "Level:", x=WINDOWWIDTH - 8, y=5, fgcolor="gray", bgcolor="black"
    )
    WINDOWSURF.putchars(
        str(level), x=WINDOWWIDTH - 7, y=6, fgcolor="white", bgcolor="black"
    )
...
def drawNextPiece(piece):
    # draw the "next" text
    WINDOWSURF.putchars(
        "Next:", x=WINDOWWIDTH - 8, y=8, fgcolor="gray", bgcolor="black"
    )
